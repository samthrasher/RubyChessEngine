...
class Level4ComputerPlayer < Level3ComputerPlayer
  ...
  def play_turn
    ...
    start_time = Time.now
    move = prevent_capture_move || capture_move || random_move
    print "Thought for #{Time.now - start_time} seconds"
  end
...
class EngineComputerPlayer
  ...
  def display=(display)
    @display = display
    @board = display.board
    @engine = ChessEngine.new(board, color)
  end
  ...
  def best_move
    ...
    pos = {board: board, color: color}
    start = Time.now
    valid_moves = board.all_moves(color)
    move = engine.best_moves(pos, depth).find {|mv| valid_moves.include?(mv[:move])}
    move[:move]
    # while Time.now - start < MIN_THINK_TIME
    #
    #   # depth += 1
    #   # p depth
    # end
